  ...
  render() {
    ...
    events.forEach(event => {
      if (inRange(event, start, end, accessors)) {
        const eTmpStart = accessors.start(event);
        const eTmpEnd = accessors.end(event);
        const eStart = eTmpStart ? new Date(eTmpStart) : null;
        const eEnd = eTmpEnd ? new Date(eTmpEnd) : null;
        if (
