...
import { getWindowDimension, IdDeviceBreakpointsByWidth, IdMobileHeight } from './utilResponsive';
...
export class Responsive extends React.PureComponent {
  ...
  shouldRender = (display, width, height) => {
    ...
    if (
      ...
      (width <= IdDeviceBreakpointsByWidth.tablet_max && width >= IdDeviceBreakpointsByWidth.tablet_min)
    ) {
      return true;
    }
    ...
    if (
      ...
      (width <= IdDeviceBreakpointsByWidth.mobile_max && height >= IdMobileHeight.mobileLandscape_max)
    ) {
      return true;
    }
    return !!(
      display.indexOf('MobileLandScape') !== -1 &&
      (width <= IdDeviceBreakpointsByWidth.mobile_max && height <= IdMobileHeight.mobileLandscape_min)
    );
  };
}
